...
export default function Patients() {
  ...
  const [sortBy, setSortBy] = useState('id');       // default sort column = id
  ...
  const fetchPatients = async () => {
    try {
      ...
      // If you want search to be handled by backend, uncomment:
  ...
  // Optional: debounced search (if moving search to backend)
  // For now, we keep frontend search filtering (but no re-sorting)
  ...
  // NO local .sort() – backend order is respected
  ...
  return (
    <Box sx={{ p: 3, bgcolor: '#f5f7fb', minHeight: '100vh' }}>
      ...
      {/* Toolbar */}
      <Paper sx={{ p: 2, mb: 2, borderRadius: 3, display: 'flex', flexWrap: 'wrap', gap: 2, alignItems: 'center', justifyContent: 'space-between' }}>
        <TextField
        ...
        <Box display="flex" gap={2} flexWrap="wrap">
          ...
          {canAdd && (
            <Button
              ...
              sx={{ borderRadius: 3, px: 3, textTransform: 'none', fontWeight: 600, boxShadow: '0 8px 20px rgba(25,118,210,0.25)' }}
            >
              Add Patient
            </Button>
          )}
          ...
          <TableBody>
            {filteredPatients.length > 0 ? (
              filteredPatients.map((p) => (
                <TableRow key={p.id} hover sx={{ transition: 'all 0.2s ease-in-out', '&:hover': { backgroundColor: '#f3f8ff' } }}>
                  ...
                  {canModify && (
                    <TableCell align="center">
                      <IconButton onClick={() => handleEdit(p)} sx={{ color: '#1976d2', '&:hover': { bgcolor: '#e3f2fd' } }}>
                        <EditIcon />
                      </IconButton>
                      <IconButton onClick={() => handleDelete(p.id)} sx={{ color: '#d32f2f', '&:hover': { bgcolor: '#ffebee' } }}>
                        <DeleteIcon />
                      </IconButton>
                    </TableCell>
                  )}
